...
if ! command -v docker >/dev/null 2>&1; then
  echo "[build_esp32_m5] ❌ docker command not found. Please install Docker and try again." >&2
  exit 1
fi

if [ ! -d "${ENGINE_ROOT}" ]; then
  echo "[build_esp32_m5] ❌ Unable to locate robotick-engine at '${ENGINE_ROOT}'. Set ROBOTICK_ENGINE_SOURCE_DIR to the engine checkout." >&2
  exit 1
fi

if ! docker image inspect "${IMAGE}" >/dev/null 2>&1; then
  echo "[build_esp32_m5] Pulling Docker image ${IMAGE}..."
  docker pull "${IMAGE}"
fi

echo "[build_esp32_m5] Starting ESP32+M5 build using ${IMAGE}..."
docker run --rm \
  --user root \
...
export ROBOTICK_PLATFORM_ESP32_M5=1
set -x
...
echo "[build_esp32_m5] ✅ ESP32+M5 build (esp32s3) finished successfully."
