...
namespace robotick
{
	...
	struct TemporalGroupingWorkload
	{
		...
		void tick(const TickInfo& tick_info)
		{
			...
			struct Cand
			{
				TemporalGroupingResult res;
			};
			FixedVector<Cand, 16> pool;

			const float min_f = robotick::TemporalGrouping::clampf(config.f0_min_hz, config.fmin_hz, config.fmax_hz);
			const float max_f = robotick::TemporalGrouping::clampf(config.f0_max_hz, config.fmin_hz, config.fmax_hz);
			...
			for (uint8_t ksel = 0; ksel < K; ++ksel)
			{
				...
				for (float f0 = min_f; f0 <= max_f; f0 *= step)
				{
					...
					if (c.res.amplitude < config.min_amplitude)
						continue;
					if (!single_ridge_ok && c.res.amplitude < min_rel * frameE)
						continue;
					if (c.res.harmonicity < config.min_harmonicity)
						continue;
				...
				const uint8_t cap2 = (config.history_frames > MaxHistory) ? MaxHistory : config.history_frames;
