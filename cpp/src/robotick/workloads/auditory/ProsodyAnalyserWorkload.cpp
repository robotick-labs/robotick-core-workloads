...
namespace robotick
{
	struct ProsodyAnalyserConfig
	{
		...
		float speaking_rate_decay = 0.9f; // EMA smoothing

		float min_pitch_hz = 60.0f;	 // a very deep adult voice
		float max_pitch_hz = 600.0f; // a very high childs voice
	};
	...
	struct ProsodyAnalyserWorkload
	{
		...
		{
			return (std::fabs(denominator) > 1e-12f) ? numerator / denominator : fallback;
		}
		...
		void tick(const TickInfo& info)
		{
			...
			prosody.voiced = voiced_now;
			prosody.voiced_confidence = voiced_now ? 1.0f : 0.0f;

			// --- Basic pitch and slope ---
			...
			if (previous_pitch > 0.0f && current_pitch > 0.0f)
			{
			...
			if (pitch_info.harmonic_amplitudes.size() >= 2)
			{
				...
				prosody.spectral_brightness = static_cast<float>(-20.0 * slope); // positive = brighter
			}
			else
			{
			...
			// --- Jitter & shimmer (very rough proxies) ---
